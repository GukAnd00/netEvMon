...
async function getMachines({ filter }) {
  ...
  if(filter._id) {
    ...
    try {
      machines = await machineRepo.findById({ filter, select: "name ipAddr  health numberOfPacketsPerPeriod lastActivity" });
    } catch (e) {
  ...
  try {
    machines = await machineRepo.findByFilter({ filter, select: "name ipAddr  health numberOfPacketsPerPeriod lastActivity history", skip, limit });
  } catch (e) {
